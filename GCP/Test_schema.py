        ...
        for column in columns:
            ...
            if column["type"] == "STRING" and column["length"]:
                ...
                bq_field = bigquery.SchemaField(
                    ...
                    max_length=column['length']  # Set max_length for STRING type
                )
            elif column["type"] in ["NUMERIC", "DECIMAL"] and column["precision"]:
                bq_field = bigquery.SchemaField(
                    name=column["name"],
                    field_type=field_type,
                    mode="NULLABLE" if column["nullable"] else "REQUIRED",
                    description=description,
                    precision=column["precision"],  # Set precision for NUMERIC/DECIMAL types
                    scale=column["scale"] if column.get("scale") else None  # Optional: Set scale for NUMERIC/DECIMAL types
                )
            else:
            ...
            bigquery_schema[table].append(bq_field)
...
def main():
    ...
    # Save SQL Server schema to file (optional)
    ...
    # Save BigQuery schema to file (optional)
